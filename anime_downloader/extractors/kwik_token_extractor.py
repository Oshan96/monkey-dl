...
def PyJsHoisted__0xe12c_(d, e, f, this, arguments, var=var):
    ...
    def PyJs_anonymous_0_(a, b, c, this, arguments, var=var):
        ...
        if PyJsStrictNeq(var.get('h').callprop('indexOf', var.get('b')),(-Js(1.0))):
            return var.put('a', (var.get('h').callprop('indexOf', var.get('b'))*var.get('Math').callprop('pow', var.get('e'), var.get('c'))), '+')
    PyJs_anonymous_0_._set_name('anonymous')
    var.put('j', var.get('d').callprop('split', Js('')).callprop('reverse').callprop('reduce', PyJs_anonymous_0_, Js(0.0)))
    var.put('k', Js(''))
...
def PyJsHoisted_extract_data_(h, u, n, t, e, r, this, arguments, var=var):
    ...
    while (var.get('i')<var.get('len')):
        try:
            ...
            var.put('r', var.get('String').callprop('fromCharCode', (var.get('_0xe12c')(var.get('s'), var.get('e'), Js(10.0))-var.get('t'))), '+')
        finally:
                (var.put('i',Js(var.get('i').to_number())+Js(1))-Js(1))
